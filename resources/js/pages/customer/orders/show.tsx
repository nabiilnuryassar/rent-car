...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
    return (
        <div className="flex items-start justify-between gap-4 border-b border-slate-gray/10 py-3 last:border-0">
            <dt className="shrink-0 text-xs font-semibold text-slate-gray">{label}</dt>
            <dd className="text-right text-xs font-bold text-navy-blue">{value}</dd>
        </div>
    );
}
            ...
            <div className="mx-auto max-w-2xl">
                ...
                <div className="mb-5 overflow-hidden rounded-2xl border border-slate-gray/10 bg-base-white shadow-sm">
                    <div className="flex items-center justify-between gap-4 p-5 sm:p-6">
                        <div>
                            ...
                            <p className="mt-1 text-2xl font-extrabold text-navy-blue sm:text-3xl">
                                {formatCurrency(order.total_amount)}
                            </p>
                        </div>
                        <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold sm:px-4 sm:py-2 sm:text-sm ${style.bg} ${style.text}`}>
                    ...
                    <div className="grid grid-cols-4 border-t border-slate-gray/10">
                        {[
                            ...
                            const statusOrder = [
                                ...
                                'ready_to_dispatch', 'ongoing', 'waiting_overtime_payment',
                                'completed',
                            ];
                            ...
                            return (
                                <div
                                    key={step.key}
                                    className={`flex flex-col items-center gap-1 py-3 text-center text-[10px] font-bold transition-colors ${
                                        isDone ? 'text-navy-blue' : 'text-slate-gray/40'
                                    } ${i < arr.length - 1 ? 'border-r border-slate-gray/10' : ''}`}
                                >
                                    <span className={`h-2 w-2 rounded-full ${isDone ? 'bg-navy-blue' : 'bg-slate-gray/20'}`} />
                                    {step.label}
                                </div>
                            );
                        })}
                ...
                <div className="mb-5 flex items-center gap-4 rounded-2xl border border-slate-gray/10 bg-base-white p-4 shadow-sm sm:p-5">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-gray">
                ...
                <div className="mb-5 rounded-2xl border border-slate-gray/10 bg-base-white p-5 shadow-sm sm:p-6">
                    <h3 className="mb-1 text-xs font-bold uppercase tracking-wider text-slate-gray">
                        Detail Pesanan
                    </h3>
                    <dl>
                        <DetailRow
                            label="Mulai"
                            value={new Date(order.start_at).toLocaleString('id-ID', {
                                day: 'numeric', month: 'short', year: 'numeric',
                                hour: '2-digit', minute: '2-digit',
                            })}
                        />
                        <DetailRow
                            label="Selesai (rencana)"
                            value={new Date(order.end_at).toLocaleString('id-ID', {
                                day: 'numeric', month: 'short', year: 'numeric',
                                hour: '2-digit', minute: '2-digit',
                            })}
                        />
                        {order.actual_return_at && (
                            <DetailRow
                                label="Dikembalikan"
                                value={new Date(order.actual_return_at).toLocaleString('id-ID', {
                                    day: 'numeric', month: 'short', year: 'numeric',
                                    hour: '2-digit', minute: '2-digit',
                                })}
                            />
                        ...
                        {order.delivery_address && (
                            <DetailRow label="Alamat Pengantaran" value={order.delivery_address} />
                        )}
                        <DetailRow
                            label="Luar Kota"
                ...
                {/* Driver card */}
                <div className="mb-5 rounded-2xl border border-slate-gray/10 bg-base-white p-5 shadow-sm sm:p-6">
                    <h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-gray">
                        Pengemudi
                    </h3>
                    {order.driver ? (
                        ...
                        <div className="flex items-center gap-3 rounded-xl bg-surface-gray px-4 py-3">
                            <User className="h-5 w-5 shrink-0 text-slate-gray/50" aria-hidden="true" />
                            <p className="text-xs font-semibold text-slate-gray">
                                Pengemudi akan ditugaskan setelah pesanan diproses.
                            </p>
                        </div>
                    )}
                </div>
                ...
                {pendingPayment && (
                    <div className="mb-5 rounded-2xl border border-amber-gold/30 bg-amber-gold/5 p-5 shadow-sm sm:p-6">
                        ...
                        <div className="mb-4 flex items-center gap-3 rounded-xl border border-amber-gold/20 bg-base-white px-4 py-3">
                            <Calendar className="h-4 w-4 shrink-0 text-amber-gold" aria-hidden="true" />
                            <p className="font-mono text-xs font-bold text-navy-blue">
                                BCA: 1234567890 a.n. PT URBAN 8 Indonesia
                            </p>
                        </div>
                        <label className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-navy-blue px-6 py-3 text-sm font-bold text-amber-gold shadow-md transition-all hover:-translate-y-0.5 hover:bg-navy-blue/90">
                ...
                {waitingVerification && (
                    <div className="mb-5 flex items-start gap-3 rounded-2xl border border-orange-200 bg-orange-50 p-5">
                        ...
                        <p className="text-sm font-semibold text-orange-700">
                            Bukti transfer sedang diverifikasi oleh admin. Kami akan segera mengkonfirmasi pembayaran Anda.
                        </p>
                    </div>
                )}
                ...
                {paidPayment?.receipt && (
                    <div className="mb-5 rounded-2xl border border-success-green/20 bg-pale-green p-5 sm:p-6">
                        ...
                        <Link
                            ...
                            className="inline-flex items-center gap-2 rounded-full bg-success-green px-6 py-2.5 text-sm font-bold text-base-white shadow-sm transition-all hover:-translate-y-0.5 hover:bg-success-green/90"
                        >
                    ...
                    <div className="overflow-hidden rounded-2xl border border-slate-gray/10 bg-surface-gray">
                        {previewUrl ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-slate-gray">
                                <FileText className="mb-2 h-12 w-12 opacity-20" aria-hidden="true" />
                                <span className="text-xs font-medium">{selectedFile?.name}</span>
                                <span className="text-[10px] uppercase opacity-50">
                                    {selectedFile?.type.split('/')[1] ?? 'Berkas'}
                                </span>
                            </div>
                        )}
                    </div>
