...
type PricingRule = { id: number; vehicle_category_id: number; rental_unit: string; min_duration: number; max_duration: number; base_rate: number; discount_rate: number | null; category: { name: string } };
type OvertimePenalty = { id: number; vehicle_category_id: number; hourly_rate: number; category: { name: string } };
...
const unitLabels: Record<string, string> = { hour: 'Per Jam', day: 'Per Hari', week: 'Per Minggu', month: 'Per Bulan' };

export default function PricingIndex({ pricingRules, overtimePenalties, categories }: Props) {
    const [activeTab, setActiveTab] = useState<'pricing' | 'overtime'>('pricing');

    const addForm = useForm({ vehicle_category_id: '', rental_unit: 'day', min_duration: '1', max_duration: '30', base_rate: '', discount_rate: '' });
    const penaltyForm = useForm({ vehicle_category_id: '', hourly_rate: '' });

    function submitRule(e: React.FormEvent) {
        e.preventDefault();
        addForm.post(admin.pricingRules.store.url(), { onSuccess: () => addForm.reset() });
    }

    function submitPenalty(e: React.FormEvent) {
        e.preventDefault();
        penaltyForm.post(admin.overtimePenalties.store.url(), { onSuccess: () => penaltyForm.reset() });
    }

    function deleteRule(id: number) {
        if (confirm('Hapus aturan harga ini?')) {
router.delete(admin.pricingRules.destroy.url(id));
}
    }
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <div className="rounded-[20px] bg-surface-gray p-6 shadow-rental">
                        ...
                        <form onSubmit={submitRule} className="flex flex-col gap-3">
                            <select value={addForm.data.vehicle_category_id} onChange={(e) => addForm.setData('vehicle_category_id', e.target.value)} className="w-full rounded-full border border-slate-gray/20 bg-base-white px-3 py-2 text-sm">
                                <option value="">Pilih Kategori</option>
                                {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                            </select>
                            <select value={addForm.data.rental_unit} onChange={(e) => addForm.setData('rental_unit', e.target.value)} className="w-full rounded-full border border-slate-gray/20 bg-base-white px-3 py-2 text-sm">
                    ...
                    <div className="lg:col-span-2 rounded-[20px] bg-surface-gray shadow-rental overflow-hidden">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {pricingRules.length === 0 && <tr><td colSpan={5} className="px-5 py-10 text-center text-slate-gray">Belum ada aturan harga.</td></tr>}
                                {pricingRules.map((r) => (
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <div className="rounded-[20px] bg-surface-gray p-6 shadow-rental">
                        ...
                        <form onSubmit={submitPenalty} className="flex flex-col gap-3">
                            ...
                            <input type="number" placeholder="Tarif per jam (Rp)" value={penaltyForm.data.hourly_rate} onChange={(e) => penaltyForm.setData('hourly_rate', e.target.value)} className="w-full rounded-full border border-slate-gray/20 bg-base-white px-3 py-2 text-sm" />
                            <button type="submit" disabled={penaltyForm.processing} className="rounded-full bg-amber-gold py-2 text-sm font-semibold hover:bg-yellow-300 disabled:opacity-50">Tambah</button>
                        </form>
                    </div>
                    <div className="lg:col-span-2 rounded-[20px] bg-surface-gray shadow-rental overflow-hidden">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                ...
                                {overtimePenalties.map((p) => (
                                    <tr key={p.id} className="border-b border-slate-gray/20/50 hover:bg-base-white/40 transition-colors">
                                        ...
                                        <td className="px-5 py-3"><button onClick={() => deletePenalty(p.id)} className="text-xs text-red-600 hover:underline">Hapus</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
